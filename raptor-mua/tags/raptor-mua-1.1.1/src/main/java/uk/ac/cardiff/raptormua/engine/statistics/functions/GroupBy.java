...
public class GroupBy extends BaseStatistic {

    /** Class logger */
    private final Logger log = LoggerFactory.getLogger(GroupByCountDistinct.class);

    /**
     * Perform the groupByField statistic. This statistic returns a list of distinct values of the given
     * <code>groupByField</code> field. In contrast to the <code>groupByFrequency</code> method, it does not count the
     * occurrence that each distinct value occurs. It is the responsibility of this method to throw a
    ...
    public Boolean performStatistic(List<MethodParameter> methodParams, String sqlWhere)
        ...
        if (sqlWhere.equals("")) {
            query =
                    "select " + groupByField + " from " + tableName
