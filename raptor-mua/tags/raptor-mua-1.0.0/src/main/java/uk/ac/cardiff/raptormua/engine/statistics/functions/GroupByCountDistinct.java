...
public class GroupByCountDistinct extends BaseStatistic {
    ...
    public Boolean performStatistic(List<MethodParameter> methodParams, String sqlWhere)
        ...
        try {
            ...
            countDistinctField = methodParams.get(1).getValue(String.class);
        ...
        if (sqlWhere.equals("")) {
            query =
                    "select " + groupByField + ",count(distinct " + countDistinctField + ") from " + tableName
                            + " where (eventTime between ?" + " and ?) and resourceIdCategory "
                            + resourceCategoryFilter + " group by (" + groupByField + ")";
        } else {
            query =
                    "select " + groupByField + ",count(distinct " + countDistinctField + ") from " + tableName
                            + " where (eventTime between ?" + " and ?) and resourceIdCategory "
                            + resourceCategoryFilter + " and " + sqlWhere + " group by (" + groupByField + ")";
        }
