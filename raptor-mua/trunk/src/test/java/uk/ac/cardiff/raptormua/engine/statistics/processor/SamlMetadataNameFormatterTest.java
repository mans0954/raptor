...
public class SamlMetadataNameFormatterTest {
    ...
    public void instantiateSamlMetadataTest() throws Exception{
        ...
        FileBackedHTTPMetadataProvider provider = new FileBackedHTTPMetadataProvider("http://metadata.ukfederation.org.uk/ukfederation-metadata.xml",50000, "/tmp/metadata.xml");
        providers.add(provider);
        samlNameFormatter = new SamlMetadataNameFormatter(providers);
        Assert.assertNotNull(samlNameFormatter);
        System.out.println(((OrganizationDisplayName)samlNameFormatter.getProvider().getEntityDescriptor("https://www.ingentaconnect.com/shibboleth").getOrganization().getDisplayNames().get(0)).getName().getLocalString());
    }
