 ...
 * 
 */
public final class RaptorWebEngine {
    ...
    /** The currently attached MUA. */
    private MUAEntry attachedMUA;

    /** The capabilities and statistical information for the currently attached MUA. */
    ...
    public void initialise() {
        log.info("Initialising RaptorWeb...");
        for (MUAEntry entry : registry.getMUAEntries()) {
            if (entry.getIsAttached()) {
                log.info("Interrogating endpoint [{}], getting its capabilitites", entry.getServiceEndpoint());
                setAttached(entry);
            }
        }
        log.info("RaptorWeb Initialised");
    }

    /**
     * Gets the list of <code>MUAEntries</code> from the <code>MUARegistry</code>.
     * 
    ...
    public List<StatisticalUnitInformation> getStatisticalUnits() {
        ...
        if (currentlyAttachedCapabilities != null && !currentlyAttachedCapabilities.isError()) {
    ...
    public void deleteAllEntriesFromAttachedMUA(SetupModel model) {
        ...
        if (!success) {
            model.setProcessingResult("ERROR: Entries did not remove");
        } else if (success) {
            model.setProcessingResult("Operation Successful");
        }
    }
