...
public class ReportHandler {
    ...
    /** Class logger */
    final static Logger log = LoggerFactory.getLogger(ReportHandler.class);

    /** The list of {@link BaseReportConstructor}s that this report handler can use to construct reports. */
    private List<BaseReportConstructor> reportConstructors;

    public void generateReport(WebSession session, String reportType) {
        for (BaseReportConstructor reportConstructor : reportConstructors) {
    ...
    public void setReportConstructors(List<BaseReportConstructor> reportConstructors) {
        this.reportConstructors = reportConstructors;
    }

    public List<BaseReportConstructor> getReportConstructors() {
            ...
            try {
                ...
                for (File file : dirList) {
                    if (file.getName().endsWith("." + fileExtension)) {
                        String relativePath = file.getAbsolutePath().replace(baseDirectory.getFile().getAbsolutePath(), "");
