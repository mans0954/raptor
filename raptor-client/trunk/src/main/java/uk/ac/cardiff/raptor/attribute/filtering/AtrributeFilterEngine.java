...
public class AtrributeFilterEngine {
     ...
     * @return
     */
    public static List<Event> filter(AttributeFilterPolicy attributeFilterPolicy, List<Event> events){
	log.debug("Applying attribute filter policy {} to entry set",attributeFilterPolicy.getPolicyName());
	Set<Event> filteredEntries = clone(events);
    ...
    private static Set<Event> clone(List<Event> events){
