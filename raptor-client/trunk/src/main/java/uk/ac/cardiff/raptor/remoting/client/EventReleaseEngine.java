...
public class EventReleaseEngine {
    ...
    /**
     * Performs the release of all events to all applicable endpoints defined in the endpoint registry. Endpoints are
     * applicable to be sent to if they meet the push policy defined on them. Event attributes are also filtered
        ...
        for (Endpoint endpoint : endpointRegistry.getEndpoints()) {
            ...
            if (shouldRelease) {
                ...
                if (releaseSuccess == false)
                    ...
                    releaseCount++;
                    /*
                     * Saves the state of the event before modification e.g. <code>applicableEvents</code> not
    ...
    /**
     * Filters the input list of events (<code>events</code>) such that only those that are after (chronological) the
     * <code>latestPublishedEventTime</code> of the input <code>Endpoint</code> remain
     * 
    ...
    /**
     * Iterates through all push policies attached to the <code>endpoint</code> parameter to determine if events should
     * be released to this endpoint
     * 
    ...
    /**
     * Constructs an event push message, which encapsulates the events to send and some metadata about the list of
     * events.
     * 
